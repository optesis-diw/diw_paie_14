...
class HrLoan(models.Model):
    ...
    emp_account_id = fields.Many2one('account.account', string="Compte créditeur", required=True, readonly=True)
    treasury_account_id = fields.Many2one('account.account', string="Compte débiteur", required=True, readonly=True)
    ...
    def action_submit(self):
        for loan in self:
            if not self.loan_line_ids:
                raise  Warning(_('Veuillez cliquer le bouton caluler avant de soumettre'))
...
class hr_loan_type(models.Model):
        ...
        emp_account_id = fields.Many2one('account.account', string="Compte de prêt", required=False)
        treasury_account_id = fields.Many2one('account.account', string="Compte de trésorerie", required=False)
